...
@router.post("/login")
def student_login(login_data: UserLogin, db: Session = Depends(get_db)):
    """学生用户登录"""
    logger.info(f"收到学生登录请求 - 用户名: {login_data.username}")
    ...
    logger.info(f"用户 {login_data.username} (ID: {user.id}) 登录成功")
